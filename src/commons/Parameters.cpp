...
Parameters::Parameters():
        ...
        PARAM_DB_SIZE(PARAM_DB_SIZE_ID, "--db-size", "Target database size", "Effective target database size for E-value computation (in number of residues). Only work when it is set above 0, otherwise it will use the input target database size", typeid(size_t), (void *) &dbSize, "^[0-9]{1}[0-9]*$", MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
        PARAM_DEEP_SEARCH(PARAM_DEEP_SEARCH_ID, "--deep-search", "Search rnacentral and nt", "Search against rnacentral and nt databases. Make sure you write in the order of <queryDB> <rnacentral> <ntDB1> ... <ntDBn>. --strand for the rnacentral will be on default 1", typeid(bool), (void *) &deepSearch, "", MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
...
//    expand2profile.push_back(&PARAM_SEQ_ID_MODE);
    ...
    splitsearchworkflow.push_back(&PARAM_DEEP_SEARCH);
